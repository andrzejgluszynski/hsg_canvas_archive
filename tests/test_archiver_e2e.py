...
@respx.mock
async def test_module_file_linked_from_text_is_not_saved_twice(client, tmp_path):
    """A syllabus link to a module file used to create Slides.pdf and Slides-500.pdf."""
    _mount(syllabus='<p><a href="https://canvas.test/courses/1/files/500">Slides.pdf</a></p>')
    stats = await Archiver(client, tmp_path, build_html=False).run()
    course = next((tmp_path / "courses").iterdir())
    pdfs = sorted(p.name for p in (course / "files").glob("*.pdf"))
    assert pdfs == ["Slides.pdf"]
    assert stats.linked_files == 0
    readme = (course / "README.md").read_text()
    assert "canvas.test/courses/1/files/500" not in readme
    assert "Slides.pdf" in readme
    await client.aclose()


@respx.mock
async def test_second_run_does_not_create_an_id_suffixed_copy(client, tmp_path):
    ...
    course = next((tmp_path / "courses").iterdir())
    pdfs = sorted(p.name for p in (course / "files").glob("*.pdf"))
    assert "S.pdf" in pdfs
    assert "S-700.pdf" not in pdfs
    await client.aclose()


@respx.mock
async def test_two_canvas_urls_for_the_same_file_share_one_copy(client, tmp_path):
    _mount(
        page_body=(
            '<a href="https://canvas.test/courses/1/files/802">Handout.pdf</a> '
            '<a href="https://canvas.test/courses/1/files/802/preview?wrap=1">also</a>'
        )
    )
    respx.get(f"{API}/courses/1/files/802").mock(
        return_value=httpx.Response(
            200,
            json={
                "id": 802,
                ...
                "size": 3,
                "url": "https://files.test/802",
            },
        )
    )
    respx.get(url__startswith="https://files.test/802").mock(
        return_value=httpx.Response(200, content=b"PDF")
    )
    await Archiver(client, tmp_path, build_html=False).run()
    course = next((tmp_path / "courses").iterdir())
    pdfs = sorted(p.name for p in (course / "files").glob("*.pdf"))
    assert pdfs == ["Handout.pdf", "Slides.pdf"]
    leaf = (course / "pages" / "Intro.md").read_text()
    assert leaf.count("Handout.pdf") >= 2
    assert "Handout-802.pdf" not in leaf
    await client.aclose()
